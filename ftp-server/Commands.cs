...
namespace ftp_server
{
    class ClientConnection : IDisposable
    {
            ...
            else
            {
                ...
                try
                {
                    if (pathname.StartsWith("/"))
                    {
                        ...
                        newDir = Path.Combine("E:\\", pathname);
                    }
                    else
                    {
        ...
        private string HandleList(string pathname)
        {
            ...
            using (NetworkStream stream = _dataClient.GetStream())
            {
                ...
                foreach (string file in files)
                {
                    ...
                    string line = string.Format("-rw-r--r--  {0,8} {1}", f.Length, f.Name);
        ...
        {
            string current = _currentDirectory.Replace("E:\\", string.Empty).Replace('\\', '/');
        ...
        private string NormalizeFilename(string path)
        {
            ...
            {
                path = new FileInfo(Path.Combine("E:\\", path.Substring(1))).FullName;
            }
            else
            {
                try
